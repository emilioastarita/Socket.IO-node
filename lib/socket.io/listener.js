var url = require('url'),
	...
	Client = require('./client').Client,
        transports = {
          websocket:       require('./transports/websocket').websocket,
          flashsocket:     require('./transports/flashsocket').flashsocket,
          htmlfile:        require('./transports/htmlfile').htmlfile,
          "xhr-multipart": require('./transports/xhr-multipart')['xhr-multipart'],
          "xhr-polling":   require('./transports/xhr-polling')['xhr-polling']
        };
  ...
  init: function(server, options){
		...
		this.server.addListener('upgrade', function(req, socket, head){
			if (!self.check(req, socket, true, head)){
                ...
                object.merge(transports,this.options.customTransports);
		this.options.transports.forEach(function(t){
			if (t in transports){
                                if(transports[t].init) 
                                  transports[t].init(this);
			} else {
                          throw new Error('The required transport: `'+ t + '` is not loaded. Provide aditional transports in: options[\'customTransports\'][\'myCustomTransport\']');
                        } 
		}, this);
